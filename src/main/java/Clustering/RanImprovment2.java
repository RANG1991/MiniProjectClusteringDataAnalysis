...
public class RanImprovment2 {
    ...
    public RanImprovment2(Processing processing)
    {
    ...
    public double createClusters(HashMap<Integer, HashSet<Integer>> medoidsGroups)
    {
        ...
        for (Integer medoid1 : medoidsGroups.keySet()) {
            for (Integer medoid2 : medoidsGroups.keySet()) {
                if (medoid1 < medoid2) {
                    double dist = calculateDistance(medoidsGroups.get(medoid1), medoidsGroups.get(medoid2));
                    if (dist <= minDist) {
                        minMedoid1 = medoid1;
                        minMedoid2 = medoid2;
                        minDist = dist;
