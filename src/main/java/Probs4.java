...
public class Probs4 {
    ...
    public static TreeMap<Integer, HashMap<Integer, Integer>> userIdToMoviesIds = new TreeMap<>();
    ...
    public static void main(String[] args) {
    	...
    	readMoviesNames(moviesFileName);
	   	try (Stream<String> stream = Files.lines(Paths.get(ratingsFileName))) {
	          stream.forEach(Probs4::processStrings);
			  List<Integer> selectedMovies = readMoviesIdsFile(moviesIdsFileName);
			  double[] p_mArray = p_m(selectedMovies);
			  selectedMovies.forEach(x->Probs4.calculateProbs(x, selectedMovies, p_mArray));
    ...
    private static void processStrings(String str) {
        ...
        if (userIdToMoviesIds.containsKey(userID)) {
            userIdToMoviesIds.get(userID).put(movieID, rating);
        } else {
            HashMap<Integer, Integer> map = new HashMap<>();
            map.put(movieID, rating);
            userIdToMoviesIds.put(userID, map);
        }
    }
											 ...
											 )
    {
    	...
    	while (remaining.size() > 0)
    	{
            ...
            for (int movie: remaining){
            	int counter = 0;
            	for(int user : movieIdToUsersIds.get(remaining.get(index)))
            	{
            		int rate_1 = userIdToMoviesIds.get(user).get(remaining.get(index));
            		if (movieIdToUsersIds.get(movie).contains(user))
            		{
            			int rate_2 = userIdToMoviesIds.get(user).get(movie);
            			//if (rate_1==rate_2)
            			if (Math.abs(rate_1-rate_2) <= 1)
            			{
            				counter++;
            			}
            		}	
            	...
            	if (counter >= (movieIdToUsersIds.get(remaining.get(index)).size() / 2) && correlation.get(remaining.get(index)).contains(movie)){
                    cluster.add(movie);
            	}
            }
